...
from alegnn.utils.dataToolsRandom import invertTensorEW
import alegnn.utils.graphTools as graphTools #Added by Juan
...
class Trainer:
    ...
    def __init__(self, model, data, nEpochs, batchSize, **kwargs):
        ...
        if 'doSmallTrain' in kwargs.keys(): #Added by Juan
            doSmallTrain = kwargs['doSmallTrain']
        else:
            doSmallTrain = True
    ...
    def train(self):
        ...
        while epoch < nEpochs \
            ...
            while batch < nBatches \
                ...
                # # Create graph
                # self.data.maxNodes=self.data.maxNodes
                # print('nnodes',self.data.maxNodes)
                # print(type(self.data.labelID))
                # print(self.data.dataDir[0])
                # self.data.__init__(self.data.graphType,list(self.data.labelID),self.data.ratioTrain,self.data.ratioValid,self.data.dataDir,
                #                              self.data.keepIsolatedNodes, self.data.forceUndirected, self.data.forceConnected, self.data.kNN,
                #                              self.data.maxNodes,
                #                              self.data.maxDataPoints,
                #                              self.data.minRatings,
                #                              self.data.doInterpolate,
                #                              self.data.dataType, self.data.device)
                # JuanTOD): do it for GPU
                # adjacencyMatrix = self.data.getGraph().cpu().detach().numpy()
...
class TrainerSmallTrain(Trainer):
    """
    TrainerSingleNode: trainer class that computes a loss over the training set
        and runs an evaluation on a validation set, but assuming that the
        architectures involved have a single node forward structure and that the
