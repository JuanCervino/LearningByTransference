...
class _dataForClassification(_data):
    ...
    def evaluate(self, yHat, y, tol = 1e-9):
        ...
        if 'torch' in repr(self.dataType):
            ...
            totalErrors = torch.sum(torch.abs(yHat - y.type(torch.LongTensor)) > tol)
...
class MovieLens(_data):
    ...
    def __init__(self, graphType, labelID, ratioTrain, ratioValid, dataDir,
                 ...
                 maxDataPoints = None,
                 identifyID = False, categorical = True,
                 dataType = np.float64, device = 'cpu'):
        ...
        #   Label ID
        assert str(labelID).isdigit() or labelID == 'all'
        if labelID != 'all':
            assert labelID > 0
            # If it is a number, then subtract 1 to match the movieLens indexing 
        ...
        if labelID == 'all':
            ...
            # We start by counting how many rows we have
            nRows = workingMatrix.shape[0]
            ...
            nDataPoints = len(np.nonzero(workingMatrix)[0])
            ...
            #   number of data points available
                ...
                maxDataPoints = nDataPoints
            # Target number of training, valid and test samples
            ...
            rowCounter = 0
            # Save variables
            trainSignals = np.empty([0, 1, nNodes])
            trainLabels = np.empty(0)
            trainIDs = np.empty(0)
            while nTrainSoFar < nTrain and rowCounter < nRows:
               # Get the corresponding row
                thisRow = workingMatrix[randPerm[rowCounter], :]
                # Get the indices of the nonzero elements (which are the nonzero
                # columns, the nonzero nodes, which are precisely the IDs of the
                # nodes where we are going to perform the estimation)
                thisIDs = np.nonzero(thisRow)[0]
                thisNpoints = len(thisIDs)
                # Get the labels
                thisLabels = thisRow[thisIDs]
                # Get the signals
                thisSignals = np.tile(thisRow, [thisNpoints, 1])
                #   We need to zero-out those elements that will be part of
                #   the samples
                thisSignals[np.arange(thisNpoints), thisIDs] = 0
                #   Add the feature dimension (before concatenation)
                thisSignals = thisSignals.reshape([thisNpoints, 1, nNodes])
            ...
            indexTrainPoints = randPerm[0:rowCounter]
            ...
            # Save variables
            validSignals = np.empty([0, 1, nNodes])
            ...
            # We also want to know which rows we have selected so far
            indexValidPoints = randPerm[nRowsTrain:rowCounter]
            ...
            # Save variables
            testSignals = np.empty([0, 1, nNodes])
            ...
            # We also want to know which rows we have selected so far
            indexTestPoints = randPerm[nRowsTrain+nRowsValid:rowCounter]
            ...
            # But, if we want to add the IDs, we need to create the variable
            # nodeID that will be concatenated with the signals along the
            # feature dimension
            if self.identifyID is True:
                trainIDs = trainIDs.reshape([self.nTrain, 1, 1])
                trainIDs = np.tile(trainIDs, [1, 1, nNodes])
                validIDs = validIDs.reshape([self.nValid, 1, 1])
                validIDs = np.tile(validIDs, [1, 1, nNodes])
                testIDs = testIDs.reshape([self.nTest, 1, 1])
                testIDs = np.tile(testIDs, [1, 1, nNodes])
        else:
            # If we want to select a specific node (column), we first, need to 
            # check that the labelID is one of the columns (since the columns
            # index the nodes, and we want to know the estimate the value of the 
            # signal at a single node, we need to be sure that this node is
            # there)
            assert self.labelID < workingMatrix.shape[1]
            # Extract that node (recall that movieLens IDs start at 1, and
            # that this has been subtracted when redefining labelID)
            selectedLabels = workingMatrix[:, self.labelID]
            # Now we have to check the number of nonzero elements in the column
            # (the number of signals that are nonzero at that node)
            indexDataPoints = np.nonzero(selectedLabels)[0]
            # This finds all the signals (data samples) that have a nonzero 
            # element at the node of interest. From these samples we will 
            # extract the dataset.
            # The [0] is because np.nonzero() returns a tuple for the index of
            # the nonzero elements in each dimension, but here, when we select a
            # column, we have only one dimension (so [1] is empty)
            ...
            self.nTrain = round(ratioTrain * maxDataPoints) # Total train set
            self.nValid = round(ratioValid * self.nTrain) # Validation set
            self.nTrain = self.nTrain - self.nValid # Effective train set
            self.nTest = maxDataPoints - self.nTrain - self.nValid
            # Permute the indices at random
            randPerm = np.random.permutation(nDataPoints)
            # And choose the indices that will correspond to each dataset (which
            # rows, which graph signals)
            ...
            indexTestPoints = indexDataPoints[\
                              randPerm[self.nTrain+self.nValid : maxDataPoints]]
            # We zero out all the values corresponding to the label ID
            workingMatrixZeroedID = workingMatrix.copy()
            workingMatrixZeroedID[:, self.labelID] = 0.
            # Finally get the corresponding samples and store them
            trainSignals = workingMatrixZeroedID[indexTrainPoints,:]
            validSignals = workingMatrixZeroedID[indexValidPoints,:]
            testSignals = workingMatrixZeroedID[indexTestPoints,:]
            # These are setSize x nNodes signals. 
            # Add the feature dimension
            trainSignals = trainSignals.reshape(self.nTrain, 1, nNodes)
            validSignals = validSignals.reshape(self.nValid, 1, nNodes)
            testSignals = testSignals.reshape(self.nTest, 1, nNodes)
            # And store the labels (scores) as well
            trainLabels = selectedLabels[indexTrainPoints]
            validLabels = selectedLabels[indexValidPoints]
            testLabels = selectedLabels[indexTestPoints]
            # Maybe, we want to keep track of the specific node we are looking
            # at each of those. In that case, we will add that to the feature
            # dimension (i.e. the last feature corresponds to the node id)
            if self.identifyID is True:
                # Add the ID as a second element in the feature dimension
                trainIDs = self.labelID * np.ones([self.nTrain, 1, nNodes])
                validIDs = self.labelID * np.ones([self.nValid, 1, nNodes])
                testIDs = self.labelID * np.ones([self.nTest, 1, nNodes])
        ...
        if self.identifyID is True:
            trainSignals = np.concatenate((trainSignals, trainIDs), axis = 1)
            validSignals = np.concatenate((validSignals, validIDs), axis = 1)
            testSignals = np.concatenate((testSignals, testIDs), axis = 1)
        self.samples['train']['signals'] = trainSignals
        self.samples['train']['labels'] = trainLabels
        self.samples['valid']['signals'] = validSignals
        self.samples['valid']['labels'] = validLabels
        self.samples['test']['signals'] = testSignals
        self.samples['test']['labels'] = testLabels
    ...
    def createGraph(self):
        ...
        nTrainPointsAll = nTrainPointsAll-round(self.ratioValid*nTrainPointsAll)
        ...
        if (self.labelID != 'all') and (self.labelID not in nodesToKeep):
        #   So, this is interesting: we know that labelID is not part of the
        #   isolated nodes, because we checked that before. Therefore, if it
        ...
        if len(nodesToKeep) < N:
            # Update the node IDs
            #   Separate signals from IDs
            trainSignals = self.samples['train']['signals'][:,0,:]
            trainIDs = self.samples['train']['signals'][:,1,0]
            trainLabels = self.samples['train']['labels']
            validSignals = self.samples['valid']['signals'][:,0,:]
            validIDs = self.samples['valid']['signals'][:,1,0]
            validLabels = self.samples['valid']['labels']
            testSignals = self.samples['test']['signals'][:,0,:]
            testIDs = self.samples['test']['signals'][:,1,0]
            testLabels = self.samples['test']['labels']
            #   Update the ID part
            #   Train set
            ...
            #trainSignals = trainSignals.reshape(len(trainIDsToKeep), 1,
            #                                    len(nodesToKeep))
            validSignals = validSignals[validIDsToKeep][:, nodesToKeep]
            #validSignals = validSignals.reshape(len(validIDsToKeep), 1,
            #                                    len(nodesToKeep))
            testSignals = testSignals[testIDsToKeep][:, nodesToKeep]
            #testSignals = testSignals.reshape(len(testIDsToKeep), 1,
            #                                  len(nodesToKeep))
            # and the IDs
            trainIDs = newTrainIDs.reshape(len(newTrainIDs), 1, 1)
            trainIDs = np.tile(trainIDs, [1, 1, len(nodesToKeep)])
            validIDs = newValidIDs.reshape(len(newValidIDs), 1, 1)
            validIDs = np.tile(validIDs, [1, 1, len(nodesToKeep)])
            testIDs = newTestIDs.reshape(len(newTestIDs), 1, 1)
            testIDs = np.tile(testIDs, [1, 1, len(nodesToKeep)])
...
class MovieLensTransferability(_data):
    """
    MovieLens: Loads and handles handles the MovieLens-100k dataset
        The setting is that of regression on a single node of the graph. That
        is, given a graph, and an incomplete graph signal on that graph, we want
    ...
    def __init__(self, graphType, labelID, ratioTrain, ratioValid, nNodesTrain, nNodesTest, dataDir,
                 keepIsolatedNodes, forceUndirected, forceConnected, kNN,
                 maxDataPoints = None,
                 identifyID = False, categorical = True,
                 dataType = np.float64, device = 'cpu'):
        ...
        self.adjacencyMatrix = None
        self.indexDataPoints = {}
        self.nNodesTrain = nNodesTrain
        self.nNodesTest = nNodesTest
        trainNodes = np.random.choice(nodeRange,nNodesTrain,replace=False)
        listTrain = list(trainNodes)
        if self.labelID not in listTrain:
            listTrain.pop(-1)
            listTrain.append(self.labelID)
        self.trainNodes = np.asarray(listTrain)
        #print(self.trainNodes)
        listTest = listTrain
        nodeRange_list = list(np.arange(nodeRange))
        for elt in listTrain:
            nodeRange_list.remove(elt)
        listTestAddtl = []
        listTest = []
        testNodes = []
        for i in range(len(nNodesTest)):
            listTestAddtl.append(np.random.choice(nodeRange_list,nNodesTest[i]-nNodesTrain, replace=False))
            listTest.append([*listTrain, *listTestAddtl[i]])
            testNodes.append(np.asarray(listTest[i]))
        self.testNodes = testNodes
        self.idxLabelIDTrain = np.argwhere(self.trainNodes == self.labelID)
        self.idxLabelIDTrain = self.idxLabelIDTrain.squeeze()
        idxLabelIDTest = []
        for i in range(len(nNodesTest)):
            idxLabelIDTest.append(np.argwhere(self.testNodes[i] == self.labelID))
            idxLabelIDTest[i].squeeze()
        self.idxLabelIDTest = idxLabelIDTest
        ...
        if graphType == 'user':
            ...
            # rows, we have to transpose it
            workingMatrix = self.incompleteMatrix.T # Movies x User
            workingMatrix = workingMatrix[:,self.trainNodes]
            self.incompleteMatrix = workingMatrix.T
            # Now, each row is a movie score for all users, so that it is a
            # graph signal in the user-based graph.
        else:
            workingMatrix = self.incompleteMatrix[:,self.trainNodes]
            self.incompleteMatrix = workingMatrix
            # In this case, each row is a user (how that user scored all movies)
            # and this is the kind of data samples we need for movie-based
            # graphs
        ...
        if labelID == 'all':
            ...
            # We also want to know which rows we have selected so far
            indexTrainPoints = randPerm[0:rowCounter]
            self.indexTrainPoints = indexTrainPoints
            nRowsTrain = rowCounter
            ...
            # We also want to know which rows we have selected so far
            indexValidPoints = randPerm[nRowsTrain:rowCounter]
            self.indexValidPoints = indexValidPoints
            nRowsValid = rowCounter - nRowsTrain
            ...
            # We also want to know which rows we have selected so far
            indexTestPoints = randPerm[nRowsTrain+nRowsValid:rowCounter]
            self.indexTestPoints = indexTestPoints
            
            # And we also need all the data points (all the rows), so:
            indexDataPoints = np.concatenate((indexTrainPoints,
                                              indexValidPoints,
                                              indexTestPoints))
            self.idxDataPoints = indexDataPoints
            ...
            # Extract that node (recall that movieLens IDs start at 1, and
            # that this has been subtracted when redefining labelID)
            selectedLabels = workingMatrix[:, self.idxLabelIDTrain]
            # Now we have to check the number of nonzero elements in the column
            # (the number of signals that are nonzero at that node)
            indexDataPoints = np.nonzero(selectedLabels)[0]
            self.idxDataPoints = indexDataPoints
            ...
            # We zero out all the values corresponding to the label ID
            workingMatrixZeroedID = workingMatrix.copy()
            workingMatrixZeroedID[:, self.idxLabelIDTrain] = 0.
            # Finally get the corresponding samples and store them
            trainSignals = workingMatrixZeroedID[indexTrainPoints,:]
            validSignals = workingMatrixZeroedID[indexValidPoints,:]
            testSignals = workingMatrixZeroedID[indexTestPoints,:]
        ...
        # Now the data has been loaded, and the training/test partition has been
        # made, create the graph
        self.labelIDver = self.idxLabelIDTrain
        self.createGraph()
        # Observe that this graph also adjusts the signals to reflect any change
        # in the number of nodes
    ...
    def reinit(self, idx):
        ...
        self.adjacencyMatrix = None
        self.indexDataPoints = {}
#        print(self.testNodes[idx])
#        self.nNodesTrain = nNodesTrain
#        self.nNodesTest = nNodesTest
#        self.trainNodes = np.random.permutation(nNodesTrain)
#        self.testNodes = np.random.permutation(nNodesTest)
        ...
        if self.graphType == 'user':
            ...
            # rows, we have to transpose it
            workingMatrix = self.incompleteMatrix.T # Movies x User
            workingMatrix = workingMatrix[:,self.testNodes[idx]]
            self.incompleteMatrix = workingMatrix.T
            # Now, each row is a movie score for all users, so that it is a
            # graph signal in the user-based graph.
        else:
            workingMatrix = self.incompleteMatrix[:,self.testNodes[idx]]
            self.incompleteMatrix = workingMatrix
            # In this case, each row is a user (how that user scored all movies)
            # and this is the kind of data samples we need for movie-based
            # graphs
        ...
        if self.labelID == 'all':
            ...
            selectedLabels = workingMatrix[:, self.idxLabelIDTest[idx]]
            # Now we have to check the number of nonzero elements in the column
            # (the number of signals that are nonzero at that node)
            indexDataPoints = np.nonzero(selectedLabels)[0]
            # This finds all the signals (data samples) that have a nonzero 
            ...
            # the nonzero elements in each dimension, but here, when we select a
            # column, we have only one dimension (so [1] is empty)
            nDataPoints = self.nDataPoints # Total number of points

            indexTrainPoints = self.indexTrainPoints
            indexValidPoints = self.indexValidPoints
            indexTestPoints = self.indexTestPoints
            workingMatrixZeroedID = workingMatrix.copy()
            workingMatrixZeroedID[:, self.idxLabelIDTest[idx]] = 0.
            # Finally get the corresponding samples and store them
            trainSignals = workingMatrixZeroedID[indexTrainPoints,:]
            validSignals = workingMatrixZeroedID[indexValidPoints,:]
            testSignals = workingMatrixZeroedID[indexTestPoints,:]
        ...
        # Now the data has been loaded, and the training/test partition has been
        # made, create the graph
        self.labelIDver = self.idxLabelIDTest[idx]
        self.createGraph()
        # Observe that this graph also adjusts the signals to reflect any change
        # in the number of nodes
    ...
    def createGraph(self):
        ...
        isolatedNodes = np.nonzero(np.abs(diagNormalizedMatrix + 1) \
                                                                < zeroTolerance)
        if self.labelID != 'all':
            assert self.labelIDver not in isolatedNodes[0]
        #   It is highly likely that the labelID won't be isolated node, since 
        #   we are purposefully taking training samples that include this item,
        #   but just in case.
        normalizedMatrix[isolatedNodes] = 0.
        ...
        # However, if there are more connected components, maybe the labelID
        # is not part of the largest.
        if (self.labelID != 'all') and (self.labelIDver not in nodesToKeep):
        #   So, this is interesting: we know that labelID is not part of the
        #   isolated nodes, because we checked that before. Therefore, if it
        #   is not part of the largest component, it has to be part of some 
        #   other component. This implies that we do not need to check if
