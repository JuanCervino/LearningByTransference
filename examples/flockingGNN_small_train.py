...
#                                                                   #
#####################################################################
def flockingSmallTrain(maxNodes,nodesAddedPerEpoch,nodesInTest,nEpochs,saveFolder):

    thisFilename = 'flockingGNN' # This is the general name of all related files

    nAgents = maxNodes # Number of agents at training time
    # Juan modified this
    # nEpochs = nEpochs # Number of epochs
    # nodesAddedPerEpoch = 2 #Nodes that will be added per epoch
    ...
    if (nodesAddedPerEpoch==0):
        outDir= thisFilename + '-%03d-' % nAgents + saveFolder
        saveDir = saveDir + thisFilename + '-%03d-' % nAgents + saveFolder + '/normalTrain'
    else:
        outDir= thisFilename + '-%03d-' % nAgents + saveFolder
        saveDir =  saveDir + thisFilename + '-%03d-' % nAgents + saveFolder + '/smallTrain'
    ...
    # Juan Will Modify Here
    # nAgentsMax = 30 # Maximum number of agents to test the solution
    nAgentsMax =nodesInTest
    nSimPoints = 1 # Number of simulations between nAgents and nAgentsMax
        # Juan modified this, it is the other way around now
        # if nAgents = 50, nAgentsMax = 100 and nSimPoints = 1,
        # then the architectures are trained on 100 agents.
        ...
        # then the architectures are TESTED?? on 50, 75 and 100 agents.
    ...
    nRealizations = 3 # Number of data realizations
    ...
    nAgentsTest = np.linspace(nAgentsMax, nAgents, num = nSimPoints,dtype = np.int)
    print(nAgentsTest)
    ...
    for realization in range(nRealizations):
        ...
        for thisModel in modelsGNN.keys():
            ...
            # Juan Modified This
            # Create the Testing
            dataTest=[[] for i in range(nSimPoints)]
            for n in range(nSimPoints):
                ...
                posTest = dataTest[n].getData('pos', 'test')
                velTest = dataTest[n].getData('vel', 'test')
                commGraphTest = dataTest[n].getData('commGraph', 'test')
                ...
                # Full trajectory
                costOptFull[n][realization] = dataTest[n].evaluate(vel=velTest)

                # Last time instant
                costOptEnd[n][realization] = dataTest[n].evaluate(vel=velTest[:, -1:, :, :])
                ...
                thisEvalVars = modelsGNN[thisModel].evaluate(dataTest[n], **addKW)
            ...
            # Now the other epochs
            # Juan modified this
            # We re do the data with more agents
            nAgentsAux = nAgents
            for e in range(nEpochs - 1):
                nAgentsAux = nAgentsAux + nodesAddedPerEpoch
                print('Agents in total',nAgentsAux)
                #   Generate the dataset
                data = dataTools.Flocking(
                    # Structure
                    nAgentsAux,
            ...
            # Prepare the Data for the other simulation of the same agent
            data = dataTools.Flocking(
                # Structure
                nAgents,
                commRadius,
    ...
    if doFigs:
        ...
        varsPickle['costBestFull']=costBestFull
        varsPickle['costBestEnd']=costBestEnd
        varsPickle['costLastFull']=costLastFull
        varsPickle['costLastEnd']=costLastEnd
        ...
        # varsPickle['meanCostBestFull']=meanCostBestFull
        # varsPickle['meanCostBestEnd']=meanCostBestEnd
        # varsPickle['meanCostLastFull']=meanCostLastFull
        # varsPickle['meanCostLastEnd']=meanCostLastEnd
        # varsPickle['stdDevCostBestFull']=stdDevCostBestFull
        # varsPickle['stdDevCostBestEnd']=stdDevCostBestEnd
        # varsPickle['stdDevCostLastFull']=stdDevCostLastFull
        # varsPickle['stdDevCostLastEnd']=stdDevCostLastEnd
