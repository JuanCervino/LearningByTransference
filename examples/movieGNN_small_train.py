...
# #   number of ratings)
# NodesAddedPerEpoch= 50 #Nodes that will be added per epoch
# nodesInTest = 1200
# nEpochs = 13 # Number of epochs


def MovieSmallTrain(maxNodes,NodesAddedPerEpoch,nodesInTest,nEpochs,saveFolder):
    ...
    if (NodesAddedPerEpoch==0):
        outDir= thisFilename + '-' + graphType + '-' + labelIDstr + '-' + saveFolder
        saveDir = saveDir + '-' + graphType + '-' + labelIDstr + '-' + saveFolder + '/normalTrain'
    else:
        outDir= thisFilename + '-' + graphType + '-' + labelIDstr + '-' + saveFolder
        saveDir = saveDir + '-' + graphType + '-' + labelIDstr + '-' + saveFolder + '/smallTrain'
    ...
    writeVarValues(varsFile,
                   {'labelID': labelID,
                    ...
                    'maxNodes': maxNodes,
                    'NodesAddedPerEpoch': NodesAddedPerEpoch,
                    'nodesInTest' : nodesInTest,
    ...
    # Let's also do a pickle for this

    hyperparmsPickle = {}
    hyperparmsPickle['nEpochs'] = nEpochs
    hyperparmsPickle['labelID'] = labelID
    hyperparmsPickle[ 'graphType'] = graphType
    hyperparmsPickle[ 'ratioTrain'] = ratioTrain
    hyperparmsPickle[ 'ratioValid'] = ratioValid
    hyperparmsPickle[ 'maxNodes'] = maxNodes
    hyperparmsPickle[ 'NodesAddedPerEpoch'] = NodesAddedPerEpoch
    hyperparmsPickle[ 'nodesInTest'] = nodesInTest
    hyperparmsPickle[ 'minRatings'] = minRatings
    hyperparmsPickle[ 'interpolateRatings'] = interpolateRatings
    hyperparmsPickle[ 'nDataSplits'] = nDataSplits
    hyperparmsPickle['keepIsolatedNodes'] = keepIsolatedNodes
    hyperparmsPickle[ 'forceUndirected'] = forceUndirected
    hyperparmsPickle[ 'forceConnected'] = forceConnected
    hyperparmsPickle[ 'kNN'] = kNN
    hyperparmsPickle[ 'maxDataPoints'] = maxDataPoints
    hyperparmsPickle[ 'useGPU'] = useGPU
    with open(os.path.join(saveDir, 'hyperparms.pkl'), 'wb') as hyperparmsFile:
        pickle.dump(hyperparmsPickle, hyperparmsFile)
    ...
    if doLocalGNN:
        ...
        modelLclGNN['trainer'] = training.TrainerSmallTrain
    ...
    for split in range(nDataSplits):
        ...
        data = alegnn.utils.dataToolsRandom.MovieLens(graphType,  # 'user' or 'movies'
        ...
        print('Adja', type(adjacencyMatrix), adjacencyMatrix.shape)
        ...
        for thisModel in modelsGNN.keys():
            ...
            print('At epoch 0 the training graph has ', G.N, 'nodes')
            ...
            data.setNumberOfNodes(nodesInTest, labelID)
            ...
            # enforced connectedness, for instance)
            nNodes = G.N
            print('At epoch 0 the testing graph has ', G.N, 'nodes')
            # Once data is completely formatted and in appropriate fashion, change its
            # type to torch
            data.astype(torch.float64)
            # And the corresponding feature dimension that we will need to use
            ...
            # modelsGNN[thisModel].changeGSO(S, [S.shape[0]])
            print(thisModel[0:8])
            if '1Ly' in thisModel:
            ...
            costBest[modelName][split] = np.array([thisCostBest])
            costLast[modelName][split] = np.array([thisCostLast])
            ...
            for e in range(nEpochs-1):
                ...
                # Increase Size of Graph Here
                # 1 - Change Data, change learning rate start with the previous one
                # 2 - Re calculate GSO
                # 3 - Assign the new GSO to the GNN

                # Part 1 starts here
                nodesPerEpoch = nodesPerEpoch + NodesAddedPerEpoch
                data.astype('np.float')
                data.setNumberOfNodes(nodesPerEpoch, labelID)
                ...
                # enforced connectedness, for instance)
                nNodes = G.N
                print('At epoch ',e+1,' the training graph has ',G.N, 'nodes')
                # Once data is completely formatted and in appropriate fashion, change its
                # type to torch
                data.astype(torch.float64)
                # And the corresponding feature dimension that we will need to use
                ...
                thisTrainVars = modelsGNN[thisModel].train(data,
                                                           ...
                                                           **trainingOptsPerModel[modelName])
                if doFigs:
                    lossTrain[modelName][split] = np.concatenate((lossTrain[modelName][split] , thisTrainVars['lossTrain']))
                    costTrain[modelName][split] = np.concatenate((costTrain[modelName][split] , thisTrainVars['costTrain']))
                    lossValid[modelName][split] = np.concatenate((lossValid[modelName][split] , thisTrainVars['lossValid']))
                    costValid[modelName][split] = np.concatenate((costValid[modelName][split] , thisTrainVars['costValid']))
                ...
                # enforced connectedness, for instance)
                nNodes = G.N
                print('At epoch ', e+1, ' the testing graph has ', G.N, 'nodes')
                ...
                # Now check which is the model being trained
                if doFigs:
                    costBest[modelName][split] = np.append(costBest[modelName][split],thisCostBest)
                    costLast[modelName][split] = np.append(costLast[modelName][split],thisCostLast)
                # This is so that we can later compute a total accuracy with
                # the corresponding error.

                # Modify the Data to fit the original size for the following GNN

                data.setNumberOfNodes(maxNodes, labelID)
                # Once data is completely formatted and in appropriate fashion, change its
                # type to torch
                data.astype(torch.float64)
                # And the corresponding feature dimension that we will need to use
        ...
        # Set the number of Nodes of the last GNN

        data.setNumberOfNodes(maxNodes+(nEpochs-1)*NodesAddedPerEpoch, labelID)
        # Once data is completely formatted and in appropriate fashion, change its
        # type to torch
        data.astype(torch.float64)
        # And the corresponding feature dimension that we will need to use
    ...
    if doFigs and doSaveVars:
        ...
        for thisModel in modelList:
            ...
            meanCostLast[thisModel] = np.mean(costLast[thisModel], axis = 0)
            ...
            stdDevCostLast[thisModel] = np.std(costLast[thisModel], axis = 0)
        ...
        varsPickle['validationInterval'] = validationInterval
        ...
        varsPickle['meanCostLast'] = meanCostLast
        varsPickle['stdDevCostLast'] = stdDevCostLast
        ...
        # Compute the x-axis #Juan Tried This
        xTrain = np.arange(0, nEpochs * nBatches, xAxisMultiplierTrain,dtype=int)
        xValid = np.arange(0, nEpochs * nBatches, \
                              validationInterval*xAxisMultiplierValid, dtype=int)
        xTest = np.arange(nBatches,(nEpochs+1) * nBatches,nBatches)
        ...
        if xAxisMultiplierTrain > 1:
            ...
            # Go and fetch tem
            for thisModel in modelList:
                print(selectSamplesTrain,len(meanLossTrain[thisModel]\
                                                        [:]))
                ...
                stdDevCostTrain[thisModel] = stdDevCostTrain[thisModel]\
                                                            [selectSamplesTrain]
                # meanCostLast[thisModel] = meanCostLast[thisModel]\
                #                                         [selectSamplesTrain]
                # stdDevCostLast[thisModel] = stdDevCostLast[thisModel]\
                #                                             [selectSamplesTrain]
        ...
        for key in meanLossTrain.keys():
            ...
            print(xTrain.shape,meanLossTrain[key].shape, stdDevLossTrain[key].shape)
        ...
        for key in meanCostTrain.keys():
            ...
            plt.errorbar(xValid, meanCostValid[key], yerr = stdDevCostValid[key],
                         color = '#95001A', linewidth = lineWidth,
                         marker = markerShape, markersize = markerSize)
            plt.errorbar(xTest, meanCostLast[key], yerr=stdDevCostLast[key],
                         color='#156E01', linewidth=lineWidth,
                         marker=markerShape, markersize=markerSize)

            plt.ylabel(r'RMSE')
            plt.xlabel(r'Training steps')
            plt.legend([r'Training', r'Validation', r'Test'])
            plt.title(r'%s' % key)
